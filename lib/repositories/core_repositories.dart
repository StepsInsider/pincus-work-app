...
class CoreCustomerRepository {
  ...
  Future<List<CoreCustomer>> list() async {
    ...
    return (rows as List).map((r) => CoreCustomer.fromMap(Map<String, dynamic>.from(r))).toList();
  }

  Future<String?> currentCompanyId() async {
    final userId = client.auth.currentUser?.id;
    if (userId == null) return null;
    final row = await client.from('profiles').select('company_id').eq('id', userId).maybeSingle();
...
class CoreLocationRepository {
  ...
  Future<List<CoreLocation>> list({String? customerId}) async {
    ...
    return (rows as List).map((r) => CoreLocation.fromMap(Map<String, dynamic>.from(r))).toList();
  }
...
class CoreBaustelleRepository {
  ...
  Future<List<CoreBaustelle>> list() async {
    final rows = await client.from('baustellen').select().order('created_at', ascending: false);
    return (rows as List).map((r) => CoreBaustelle.fromMap(Map<String, dynamic>.from(r))).toList();
  }
