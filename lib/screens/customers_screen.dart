...
class _CustomersScreenState extends State<CustomersScreen> {
  ...
  Future<void> _loadCustomers() async {
    ...
    try {
      final customerRows = await _supabase
          .from('kunden')
          .select('id,name,firmenname,ansprechpartner,contact_person,phone,telefon,email,address,strasse,notes,company_id')
      ...
      final locations = (locationRows as List)
          .map((row) => CustomerLocation.fromMap(Map<String, dynamic>.from(row)))
  ...
  List<Customer> get _filteredCustomers {
    ...
    return _customers.where((customer) {
      ...
      final haystack = [
        ...
        customer.phone ?? '',
        customer.email ?? '',
        ...locations.map((location) => '${location.name} ${location.addressLine}'),
      ].join(' ').toLowerCase();
      ...
      builder: (context) => AlertDialog(
        ...
        content: SizedBox(
          ...
          child: SingleChildScrollView(
            child: Column(
              children: [
                TextField(controller: nameController, decoration: const InputDecoration(labelText: 'Kundenname *')),
                TextField(controller: contactController, decoration: const InputDecoration(labelText: 'Ansprechpartner')),
                ...
                TextField(controller: notesController, maxLines: 3, decoration: const InputDecoration(labelText: 'Notizen')),
              ],
            ),
          ),
        ),
          ...
          FilledButton(
            onPressed: () async {
              ...
              try {
                ...
                final profile = await _supabase.from('profiles').select('company_id').eq('id', userId).maybeSingle();
                final companyId = profile?['company_id']?.toString() ?? customer?.companyId;
                if (company == null && companyId == null) {
                  throw Exception('Dem Benutzer ist noch kein Unternehmen zugeordnet.');
                }
                ...
                if (customer == null) {
                  ...
                  await _supabase.from('kunden').update(payload).eq('id', customer.id);
                }
  ...
  Future<void> _showLocationDialog(Customer customer, {CustomerLocation? location}) async {
    final nameController = TextEditingController(text: location?.name ?? '');
    final streetController = TextEditingController(text: location?.street ?? '');
    final postcodeController = TextEditingController(text: location?.postcode ?? '');
          ...
          child: SingleChildScrollView(
            child: Column(
              children: [
                ...
                TextField(controller: nameController, decoration: const InputDecoration(labelText: 'Standortname *')),
                TextField(controller: streetController, decoration: const InputDecoration(labelText: 'Straße / Hausnummer')),
                TextField(controller: postcodeController, decoration: const InputDecoration(labelText: 'PLZ')),
                TextField(controller: cityController, decoration: const InputDecoration(labelText: 'Ort')),
                TextField(controller: notesController, maxLines: 3, decoration: const InputDecoration(labelText: 'Notizen')),
              ],
            ),
          ),
            ...
            onPressed: () async {
              ...
              try {
                ...
                if (location == null) {
                  ...
                  await _supabase.from('standorte').update(payload).eq('id', location.id);
                }
                if (context.mounted) Navigator.pop(context, true);
              } catch (error) {
                if (context.mounted) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(onPressed: _loadCustomers, tooltip: 'Aktualisieren', icon: const Icon(Icons.refresh)),
      ...
      body: _loading
          ...
          : _error != null
              ? Center(child: Padding(padding: const EdgeInsets.all(24), child: Column(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  const SizedBox(height: 12),
                  FilledButton.icon(onPressed: _loadCustomers, icon: const Icon(Icons.refresh), label: const Text('Erneut versuchen')),
                ])))
              : Column(children: [
                  Padding(
                              ...
                              itemBuilder: (context, index) {
                                ...
                                return Card(
                                  ...
                                  child: ExpansionTile(
                                    ...
                                    subtitle: Text('${locations.length} Standort${locations.length == 1 ? '' : 'e'}'),
                                    children: [
                                      ...
                                      ButtonBar(
                                        children: [
                                          TextButton.icon(onPressed: () => _showCustomerDialog(customer: customer), icon: const Icon(Icons.edit_outlined), label: const Text('Kunde bearbeiten')),
