  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [IconButton(onPressed: _loadMembers, icon: const Icon(Icons.refresh), tooltip: 'Aktualisieren')],
      ),
      body: _loading
          ...
          : _error != null
              ? Center(child: Padding(padding: const EdgeInsets.all(24), child: Column(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  const SizedBox(height: 12),
                  FilledButton.icon(onPressed: _loadMembers, icon: const Icon(Icons.refresh), label: const Text('Erneut versuchen')),
                ])))
              : _members.isEmpty
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return Card(
                            ...
                            child: ListTile(
                              ...
                              subtitle: Text(phone.isEmpty ? '$role • $status' : '$role • $phone • $status'),
                              trailing: Icon(active ? Icons.check_circle : Icons.pause_circle, color: active ? Colors.green : Colors.grey),
                            ),
                          );
                        },
                      ),
